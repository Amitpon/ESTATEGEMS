...
/** כותרת ההקשר, לפי הסיבה שהמנוע בחר את הנקודה. */
const REASON_TEXT: Record<KeyExitPoint['reason'], { title: string; note: string }> = {
  exemption: {
    title: 'מכירה בפטור ממס שבח',
    note: 'הנקודה הראשונה שבה המכירה פטורה ממס שבח - 18 חודשים מהאכלוס. מכירה לפניה חייבת במס.',
  },
  'best-return': {
    title: 'נקודת התשואה הגבוהה ביותר',
    note: 'זו אינה הדירה הראשונה ולכן אין פטור ממס שבח. מוצגת הנקודה שבה התשואה השנתית הגבוהה ביותר לפי ההנחות שלך.',
  },
  'end-of-horizon': {
    title: 'סוף תקופת הבדיקה',
    note: 'אין נקודת פטור ואין שיא ברור בטווח שנבדק.',
  },
}

/**
 * בלוק מדד בודד.
 * `hero=true`: מדד ראשי - גופן גדול יותר, font-bold. לתזרים החודשי בלבד.
 * `hero=false`: מדד משני - גופן קטן יותר, font-semibold.
 */
function Big({
  label,
  value,
